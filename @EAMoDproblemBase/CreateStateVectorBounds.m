...
% Minimum end charge
for i = 1:obj.N
    for c = 1:obj.MinEndCharge
        ub_StateVector(obj.FindEndRebLocationci(c,i)) = 0;
    end
end

% Don't create chargers/dischargers from thin air
% Prevent charging that would result in invalid charge levels i.e., outside [0,obj.C] 
for t = 1:obj.Thor
    for l = 1:obj.NumChargers
        for c = 1:obj.ChargerSpeed(l)
            % Loop does not run for obj.num_passenger_flows = 0
            for k = 1:obj.num_passenger_flows
                ub_StateVector(obj.FindDischargeLinkPtckl(t,c,k,l)) = 0;
            end
        ...
        for c = (obj.C - obj.ChargerSpeed(l) + 1):obj.C
            % Loop does not run for obj.num_passenger_flows = 0
            for k = 1:obj.num_passenger_flows
                ub_StateVector(obj.FindChargeLinkPtckl(t,c,k,l)) = 0;
            end
...
for l = 1:obj.NumChargers
    for t = (obj.Thor - obj.ChargerTime(l) + 1):obj.Thor
        for c = 1:obj.C
            % Loop does not run for obj.num_passenger_flows = 0
            for k = 1:obj.num_passenger_flows
                ub_StateVector(obj.FindChargeLinkPtckl(t,c,k,l)) = 0;
